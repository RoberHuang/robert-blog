...
class UploadsManager
{
    ...
    public function folderInfo($folder)
    {
        ...
        $slice = array_slice($breadcrumbs, -1); // 从数组中取出一段
        $folderName = current($slice);  // 返回数组中的当前单元
        $breadcrumbs = array_slice($breadcrumbs, 0, -1);

        $subfolders = [];
        foreach (array_unique($this->disk->directories($folder)) as $subfolder) {
            $subfolders["/$subfolder"] = basename($subfolder);  // 返回路径中的文件名部分
        }
        ...
        return compact(
            'folder',   // 当前文件夹的路径
            'folderName',   // 当前文件夹的名称
            'breadcrumbs',  // [ $path => $foldername ]
            'subfolders',
            'files' // 文件夹中每个文件的文件详细信息数组
        );
    }
    ...
    public function fileMimeType($path)
    {
        return $this->mimeDetect->findType(
            pathinfo($path, PATHINFO_EXTENSION) // 返回文件路径的信息
        );
    }
